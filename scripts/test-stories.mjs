...
const uncertain = [article('z-date', '2026-09-16'), article('a-time', '2026-09-16T16:00:00Z')];
assert.equal(buildStories(uncertain, now)[0].orderUncertain, true);
assert.equal(buildStories(uncertain, now)[0].articles[0].id, 'a-time', 'Date-only is not falsely treated as confirmed midnight');
assert.deepEqual(buildStories([...uncertain].reverse(), now), buildStories(uncertain, now));
assert.equal(buildStories([article('early', '2026-09-15T23:59:00Z'), ...uncertain], now)[0].orderUncertain, false);
assert.equal(buildStories([article('review', '2026-09-16', { status: 'review' })], now).length, 0);
assert.equal(buildStories([article('one', '2026-09-16')], now)[0].orderUncertain, false);
console.log('Story projection: 13 assertions passed');

...
type ManagementResponse={error?:string;articles:Article[];ai:boolean;now:number;report?:string[]};
...
export default function Admin({authorized,authorizationError,initialState,name}:{authorized:boolean;authorizationError?:string;initialState?:ManagementResponse;name:string}){
  ...
  async function refresh(){
    ...
    setArticles(data.articles);setAi(data.ai);setProjectionNow(data.now);
  }
  useEffect(()=>{
    if(!ready||initialState)return;
    const controller=new AbortController();
    fetchManagement(controller.signal).then(data=>{setArticles(data.articles);setAi(data.ai);setProjectionNow(data.now);}).catch(error=>{if(!controller.signal.aborted)setMessage((error as Error).message);});
    return ()=>controller.abort();
  },[ready,initialState]);

  async function act(action:string,actionIds=ids){
    setBusy(true);setMessage('');
    try{
      const response=await fetch('/api/manage',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,ids:actionIds})});
  ...
  return <><Header admin/><main className="shell admin-mode-shell">
    ...
    {!ready?<div className="setup-box"><h2>관리자 세션을 확인할 수 없습니다</h2><p>다시 관리자 로그인 후 시도해 주세요. 보안을 위해 계정 식별 정보는 이 화면에 표시하지 않습니다.</p></div>:<>
      ...
      <div className="admin-mode-layout">
        <AdminCommandRail published={publicArticleCount} excluded={articles.filter(article=>article.status==='excluded').length} review={articles.filter(article=>article.status==='review').length} ai={ai} busy={busy} onOpen={setPanel} onAction={act}/>
      ...
      <AdminStatusSheet status={panel} articles={articles} busy={busy} onOpenChange={open=>{if(!open)setPanel(null);}} onAction={act}/>
    </>}
  </main></>;
}

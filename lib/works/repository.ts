...
function toWork(row:WorkRow):Work{return {id:row.id,title:row.title,originalTitle:row.originalTitle,type:row.type as Work['type'],status:row.status as WorkStatus,posterUrl:row.posterUrl,posterSource:row.posterSource as Work['posterSource'],posterReferenceUrl:row.posterReferenceUrl,posterTmdbPath:row.posterTmdbPath,releaseDate:row.releaseDate,releasePrecision:row.releasePrecision as Work['releasePrecision'],officialUrl:row.officialUrl,franchise:row.franchise,seriesKey:row.seriesKey,seasonNumber:row.seasonNumber===null?null:Number(row.seasonNumber),tmdbMediaType:row.tmdbMediaType as Work['tmdbMediaType'],tmdbId:row.tmdbId===null?null:Number(row.tmdbId),tmdbSeasonNumber:row.tmdbSeasonNumber===null?null:Number(row.tmdbSeasonNumber)};}
function values(draft:WorkDraft){return {title:draft.title,originalTitle:draft.originalTitle,type:draft.type,status:draft.status,posterUrl:draft.posterUrl,...(draft.posterSource!==undefined?{posterSource:draft.posterSource,posterReferenceUrl:draft.posterReferenceUrl??null,posterTmdbPath:draft.posterTmdbPath??null}:{}),releaseDate:draft.releaseDate,releasePrecision:draft.releasePrecision,officialUrl:draft.officialUrl,seriesKey:draft.seriesKey,seasonNumber:draft.seasonNumber===null?null:String(draft.seasonNumber),...(draft.tmdbMediaType&&draft.tmdbId?{tmdbMediaType:draft.tmdbMediaType,tmdbId:String(draft.tmdbId),tmdbSeasonNumber:draft.tmdbSeasonNumber===null?null:String(draft.tmdbSeasonNumber)}:{})};}

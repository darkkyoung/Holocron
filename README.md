...
## Operations
The initial deployment is owner-private. Open `/admin`, sign in with ChatGPT and select the one-time initialization button. This permanently assigns the signed-in owner as the administrator and inserts the verified starter articles. Initialize before changing the Site audience to public. All management APIs authorize server-side with the Sites user ID when available and a versioned identity record; `HOLOCRON_ADMIN_EMAIL` is a server-configured, verified-email recovery guard for safely rebinding a legacy Site identifier. Other identities cannot replace the admin.
